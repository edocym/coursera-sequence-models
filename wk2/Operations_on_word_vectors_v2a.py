...
def cosine_similarity(u, v):
    ...
    norm_u = np.sqrt(np.sum(np.square(u)))      # same as   np.sqrt(np.dot(u, u))  or  np.sqrt(np.dot(u.T, u))
    ...
    cosine_similarity = dot / np.dot(norm_u, norm_v)     # same as    dot / (norm_u * norm_v) as both norm_u and norm_v is a scalar
    ### END CODE HERE ###
...
#     e_biascomponent = np.multiply(((np.dot(e, g)) / np.dot(g, g)), g)   # same as above
