...
def clip(gradients, maxValue):
    ...
    for gradient in [dWax, dWaa, dWya, db, dby]: # gradients:
...
def optimize(X, Y, a_prev, parameters, learning_rate = 0.01):
    ...
    # def clip(gradients, maxValue):     # return gradients
...
def model(data_x, ix_to_char, char_to_ix, num_iterations = 35000, n_a = 50, dino_names = 7, vocab_size = 27, verbose = False):
    ...
    for j in range(num_iterations):
        ...
        single_example_ix = [char_to_ix[ch] for ch in single_example] # { ch:i for i,ch in enumerate(single_example_chars) }
...
#         Y = X[1:] + [char_to_ix["\n"]]
        ...
        # def optimize(X, Y, a_prev, parameters, learning_rate = 0.01):   # return loss, gradients, a[len(X)-1]
